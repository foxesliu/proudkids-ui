    ...
    <el-row type="flex" justify="center" class="basic-set">
      <el-col :span="4">
        <div class="basic-list">
          <span>{{$t('i18nView.ehTeachPool.language')}}：</span><span>{{formData.language === '英语' ? 'English' : '中文'}}</span>&emsp;
          <el-switch
      ...
      <el-col :span="4">
        <div class="basic-list">
          ...
          <span v-show="formData.sex==true&&isEnglish==true">Male</span>
          <span v-show="formData.sex==false&&isEnglish==true">Female</span>
      ...
      <el-col :span="4">
        <div class="basic-list">
          ...
          v-if="formData.pk >= 12">{{$t('i18nView.ehTeachPool.year')}}</span>{{formData.pk |
          monthToMonthFilter}}{{$t('i18nView.ehTeachPool.month')}}
      ...
      <el-col :span="4">
        <div class="basic-list border-r">
          ...
          <span
            v-if="formData.teachAge >= 12">{{$t('i18nView.ehTeachPool.year')}}</span>{{formData.teachAge |
          monthToMonthFilter}}{{$t('i18nView.ehTeachPool.month')}}
    ...
    <el-row class="educations">
      ...
      <ul class="education">
        <li v-for="(item,index) in educationData"
            ...
            :class="sanfengDegree.indexOf(item)>-1 ? 'active':''">
          {{item}}
        </li>
      ...
      <div class="box">
        <div class="pdf-box" id="content" @click="showPdfDownload=false">
          ...
          <div class="bottom">
            <div class="infor">
              <div class="p">
                <div><span>Language:</span><span
                  class="color-oran ml-10">{{formData.language === '英语' ? 'English' : '中文'}}</span></div>
            ...
            <div class="imgs">
              ...
              <div><img :src="imgs[0]['url']" id="cavasimg1" /></div>
              <div><img :src="imgs[1]['url']" id="cavasimg2" /></div>
              <div><img :src="imgs[2]['url']" id="cavasimg3" /></div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        initMoth: null,
        language: ['English', '中文'],
        educationData: ['Bachelors', 'Masters', 'Doctorate/PhD', 'TEFL/TESOL', 'CELTA', 'IELTS', 'BED/Teaching Certification', 'Other'],
        sanfengDegree: [],
...
//      },
      ...
      download(src) {
        this.downloadIamge('#cavasimg1', 'cavasimg1');
        this.downloadIamge('#cavasimg2', 'cavasimg2');
        this.downloadIamge('#cavasimg3', 'cavasimg3');
        //return;
//        var canvas = document.getElementById("cavasimg")
//        var context = canvas.getContext("2d")
//
//        var img = new Image()
//        img.src = './pdf-topbg.gif'
////图片加载完后，将其显示在canvas中
//        img.onload = function(){
...
//        ctx.drawImage(import('./pdf-topbg.gif'), 0, 0,210,280);
////        //创建image对象
////        var imgObj = new Image();
////        imgObj.src = import('./pdf-topbg.gif')
////        imgObj.setAttribute("crossOrigin", 'Anonymous')
////        //待图片加载完后，将其显示在canvas上
////        imgObj.onload = function () {
////          var ctx = cvs.getContext('2d');
////          ctx.drawImage(this, 0, 0,210,280);//this即是imgObj,保持图片的原始大小：470*480
...
////        }
      ...
      getData() {
        getTeacherDetails(this.TeaUserId, this.teacherType).then(response => {
          ...
          this.sanfengDegree = (this.formData.degree).split('|||');
          console.log(this.sanfengDegree)
      ...
      selectAducation(item, index) {
        ...
        if (this.adu_teacher_adit) {
          if (this.sanfengDegree.indexOf(item) > -1) {
            sfTools.arrayRemoveValue(this.sanfengDegree, item);
          } else {
            this.sanfengDegree.push(item);
          }
          console.log(this.sanfengDegree)
          this.formData.degree = this.sanfengDegree.join('|||');
