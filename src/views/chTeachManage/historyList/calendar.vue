  ...
  <div class="calendar">
    ...
    <table cellpadding="5">
      ...
      <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
        <td v-for="(child,k2) in day" v-if="flag"
            :class="{'selected':child.selected,'disabled':k2==6||k2==5||k2==4||k2==3||k2==2||child.disabled}"
            @click="select(k1,k2,$event,days)">
          <span :class="{'red':k2==0||k2==1}">{{child.day}}</span>
        ...
        <td v-for="(child,k2) in day" v-if="!flag"
            :class="{'selected':child.selected,'disabled':k2==6||k2==0||k2==1||child.disabled}"
            @click="select(k1,k2,$event,days)">
          <span :class="{'red':k2==2||k2==3||k2==4||k2==5}">{{child.day}}</span>
          <div class="text" v-if="child.eventName!=undefined">{{child.eventName}}</div>
          <div class="text"
               :class="{'isLunarFestival':child.isLunarFestival,'isGregorianFestival':child.isGregorianFestival}"
               v-if="lunar">{{child.lunar}}
  ...
  export default {
    ...
    created() {
      ...
      if (roleId == 20 || roleId == 21 || roleId == 22 || roleId == 31) {//中教
        this.flag = true
      } else if (roleId == 23 || roleId == 24 || roleId == 5 || roleId == 32) {//外教
        this.flag = false
      }
    },
    computed: {
