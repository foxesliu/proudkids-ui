<template>
    ...
    <div class="main-box">
        ...
        <div class="table-box">           
            <el-table :data="table" border style="width: 100%">
               <el-table-column
                ...
                <template slot-scope="scope">
                        <span>
                            {{scope.row.jsonObj.startTime | parseTime('{y}-{m}-{d}') || '/'}}
                        </span>
                    </template>
                </el-table-column>
                <el-table-column
                ...
                label="停课结束时间" align="center">
                    <template slot-scope="scope">
                        <span>
                            {{scope.row.jsonObj.endTime | parseTime('{y}-{m}-{d}') || '/'}}
                        </span>
                    </template>
                </el-table-column>
                <!-- <el-table-column
                
                label="停课课时" align="center">
                    <template slot-scope="scope">
                        <span>
                            {{scope.row.jsonObj.periods }}
                        </span>
                    </template>
                </el-table-column> -->
        ...
        <div class="pay-details border">
            <p class="reson"><span>备注：</span>{{mes}}</p>
    ...
    export default {
        ...
        created(){
            ...
            .then(res => {
                ...
                console.log(JSON.parse(res.data.data.billJson))
                ...
                if(this.data.stat == 2){
                    this.show2 = false
                    this.title = '停课工单>流程中>工单详情'
                }
                if(this.data.stat == 3){
                    this.title = '停课工单>待审核>工单详情'
                }
                if(this.data.stat == -1){
                    this.show = false
                    this.reson = JSON.parse(this.data.resRemark).remark1
                    this.title = '停课工单>未通过>工单详情'
                }
                if(this.data.stat == 1){
                    this.title = '停课工单>已通过>工单详情'
                 }
                 if(this.data.stat == -2){
                    this.show3 = false
                    this.resRemark = this.data.resRemark
                    this.title = '停课工单>异常工单>工单详情'
                 }
