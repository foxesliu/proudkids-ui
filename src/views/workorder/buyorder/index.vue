    ...
    export default {
        ...
        created() {
            ...
            .then(res => {
               ...
               //this.nums.push=res.data.data[{stat: 3, num: 0, type: "2"}]
        ...
        methods: {
            ...
            gotopage(item) {
                if(item.stat == 3){
                    ...
                    this.$router.push({path: '/workorder/confirm',query:{stat:item.stat}})
                } 
                if(item.stat == 2){
                    
                    this.$router.push({path: '/workorder/billList',query:{stat:item.stat,title:this.title,type:this.type}})
                } 
                if(item.stat == -1){
                    this.$router.push({path: '/workorder/billList',query:{stat:item.stat,title:this.title,type:this.type}})
                } 
