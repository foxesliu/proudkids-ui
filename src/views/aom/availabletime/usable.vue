  ...
  <div class="aom-container">
    <div class="p-20">
      <div class="flex-box header">
        ...
        <div class="flex-box">
          <el-button type="success" size="small" style="height:36px;width:180px;" class="mr" v-if="this.roleId != 27">Holiday Applyment  </el-button>
          <el-button type="primary" size="small" style="height:36px;width:90px;" class="mr" @click="gotoHistory()">History</el-button>
          ...
          <div class="o-hidden">
            <ul class="flex-box">
              <li v-for="(item,index) in list7" :key="index">
                ...
                <p :class="item.isAssignedClass==false?'green':'red'" v-if="item.stat == 1">{{item.status}}</p>
                <p :class="item.isAssignedClass==false?'blue':'blue'" v-if="item.stat == 0">{{item.status}}</p>
                <div :class="item.stat===1?'t-center':'t-center active'"  @click="setUsable(item.stat,item.availableTimeId,item.tutorUptimeId,item.status)">{{item.stat === 1 ? 'Available' : 'Unavailable'}}</div>
              </li>
            </ul>
          </div>
        </div>
...
<script>
  import {tutorUptimeWeekPoint, tutorUptimeAvailable, tutorUptimeDisavailable} from '../../../api/aom.js';
  ...
  export default {
    ...
    methods: {
      ...
      gotoHistory(){
        this.$router.push({
           path: '/chTeachManage/historyList',
      ...
      _tutorUptimeWeekPoint111(){
        tutorUptimeWeekPoint(7, this.teacherId,this.originTimeGe,this.type).then((res) => {
          this.list7 = res.data.timeSoltInfo;
          this.changeStatus(this.list7)
          console.log(this.list7);
        });
        tutorUptimeWeekPoint(6, this.teacherId,this.originTimeGe,this.type).then((res) => {
          this.list6 = res.data.timeSoltInfo;
          this.changeStatus(this.list6)
          console.log(this.list6);
        });
        tutorUptimeWeekPoint(1, this.teacherId,this.originTimeGe,this.type).then((res) => {
      ...
      _tutorUptimeWeekPoint222(){
        tutorUptimeWeekPoint(2, this.teacherId,this.originTimeGe,this.type).then((res) => {
          this.list2 = res.data.timeSoltInfo;
          this.changeStatus(this.list2)
          console.log(this.list2);
        });
        tutorUptimeWeekPoint(3, this.teacherId,this.originTimeGe,this.type).then((res) => {
          this.list3 = res.data.timeSoltInfo;
          this.changeStatus(this.list3)
          console.log(this.list3);
        });
        tutorUptimeWeekPoint(4, this.teacherId,this.originTimeGe,this.type).then((res) => {
          this.list4 = res.data.timeSoltInfo;
          this.changeStatus(this.list4)
          console.log(this.list4);
        });
        tutorUptimeWeekPoint(5, this.teacherId,this.originTimeGe,this.type).then((res) => {
