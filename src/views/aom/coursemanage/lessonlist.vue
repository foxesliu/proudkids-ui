      ...
      <div class="course-container">
        <el-table :data="list" border style="width:100%"  :header-cell-style="{background:'#f9fafc'}">
          ...
          <el-table-column label="预习精美文字" align="center">
            <template slot-scope="scope">
              <el-button size="mini" type="success" @click="gotoMessageClass(scope.row)" style="width:90px">进入
              </el-button>
            </template>
          </el-table-column>
          ...
          <el-table-column label="作业精美文字" align="center">
            <template slot-scope="scope">
              <el-button size="mini" type="primary" @click="gotoMessageHomeWork(scope.row)" style="width:90px">进入
              </el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    gotoBeforeclass(row) {
      console.log(row);
      this.$router.push({
        path: "/aom/coursemanage-question",
        query: { courseId: row.courseId, lessonId: row.lessonId,mold:0,courseName:row.name}
      });
    },
    ...
    gotoAfterclass(row) {
      ...
      this.$router.push({
        path: "/aom/coursemanage-question",
        query: { courseId: row.courseId, lessonId: row.lessonId,mold:1,courseName:row.name}
      });
    },

    gotoMessageClass (row) {
      this.$router.push({
        path: "/aom/coursemanage-message",
        query: { courseId: row.courseId,lessonId: row.lessonId, mold:0,courseName:this.courseName,lessonName:row.name }
      });
    },
    gotoMessageHomeWork (row) {
      this.$router.push({
        path: "/aom/coursemanage-message",
        query: { courseId: row.courseId,lessonId: row.lessonId, mold:1,courseName:this.courseName,lessonName:row.name }
      });
    }
  },
  mounted() {}
