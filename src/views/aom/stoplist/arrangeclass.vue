<template>
  <div class="aom-container">
    <div class="p-20">
      <v-pagetitle title="停课名单 > 停课结束 > 排班"></v-pagetitle>
      <div class="addclass-box">
        <div class="item pb-20">
          <div class="h-60 border-bottom font18">排班</div>
          ...
          <div class="search-box o-hidden border-top mt-15 pt-15">
            <div class="pull-left">
              <div>
                <el-select class="width-150" v-model="formInline1.getTime" placeholder="班级创建/分配时间">
                ...
                <el-select class="width-150" v-model="formInline3.getTime" placeholder="班级结束日期">
                  <el-option label="由近到远" value="由近到远"></el-option>
                  <el-option label="由远到近" value="由远到近"></el-option>
                  <el-date-picker v-model="formInline3.getTime" align="left" type="date" placeholder="选择具体日期"
                                  value-format="yyyy-MM-dd" :picker-options="pickerOptions1">
                  </el-date-picker>
                </el-select>
                <el-select v-model="obj.classTypeId" class="width-150" placeholder="班级类型名称">
                  <el-option v-for="item in classTypeList" :key="item.classTypeId" :label="item.name"
                             :value="item.classTypeId"></el-option>
                </el-select>
              </div>
              <div class="mt-10">
                <el-select class="width-150" v-model="obj.currentNumber" placeholder="班级人数">
                  <el-option v-for="item in classNumberArray" :key="item" :label="item"
                             :value="item"></el-option>
                </el-select>
                <el-select class="width-150" v-show="weekTimeItems.length>0" v-model="obj.weektimeId"
                           placeholder="每周上课时间">
                ...
                <el-input class="width-150" v-model="obj.name" placeholder="班级名称"/>
              </div>
            </div>
            <div class="pull-right">
              <el-button type="success" class="width-100" @click="searchClasses">查询</el-button>
          ...
          <div class="item">
            <div class="h-80 border-bottom o-hidden">
              ...
              <div class="pull-left">
                <el-date-picker class="width-300" v-model="chooseDate" type="date" placeholder="选择日期">
                </el-date-picker>
              </div>
            </div>
          </div>
          ...
          <div class="item o-hidden t-center">
            <div class="pt-10 pb-10">
              ...
              <el-button type="success" :disabled="btnIsDisabled"
                         class="width-200" @click="submit">确认
              </el-button>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      this.periodNum = this.$route.query.periodNum;
      this.classRuserSuspendId = this.$route.query.classRuserSuspendId;
    ...
    methods: {
      ...
      submit() {
        var submitObj = {
          ...
          classRuserSuspendId: this.classRuserSuspendId
        };
        if (submitObj.classId == '') {
          this.$message({type: 'error', message: '请选择要加入的班级'});
          return false;
        }
        if (submitObj.startDate == '') {
