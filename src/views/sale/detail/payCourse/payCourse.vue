...
//        })
        ...
        if (this.showHandOrOnline) {
          this.formData.money = this.handList.reduce(function (prev, cur) {  //总价格
            return prev + (cur.money);
          }, 0)
        } else {
          this.formData.money = this.handList2.reduce(function (prev, cur) {  //总价格
            return prev + (cur.money);
          }, 0)
        }
      ...
      payClass() { //购买课程
        ...
        //return;
        payClass(this.formData, this.classTypeId, this.formData.orderType).then((res) => {
          var that = this;
          this.$notify({
