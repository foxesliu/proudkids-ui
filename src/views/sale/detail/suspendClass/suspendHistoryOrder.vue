<template>
  <div class="presentCourseHistory">
    <el-table :key='tableKey' :data="tableData" v-loading="listLoading" element-loading-text="正在加载中" border fit
      ...
      <el-table-column align="center" label="停课开始时间" show-overflow-tooltip>
        <template slot-scope="scope">
          <span>
      ...
      <el-table-column align="center" label="停课结束时间" show-overflow-tooltip>
        <template slot-scope="scope">
          <span>
            {{scope.row.billFlowHistoryVo.createTime || '/'}}
          </span>
    ...
    methods: {
      getList() {
        ruserOrdersHistoryDetail(this.phone, this.flowId).then((res) => {
          ...
          const {ruserOrdersVo, billFlowHistoryVo, classesVo, billsAuditHistories, billsAuditHistoryVo} = res.data.data
