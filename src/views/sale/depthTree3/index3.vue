<template>
  <el-row style="width: 100%">
    <el-col style="width: 100%">
      <el-row style="float:left;margin-right:10px;">
        <el-select
...
<script>
  ...
  export default {
    ...
    watch: {
      //切换大区
      area: function (val) {
        this.$emit('clickIndex', 0);
        this.names.name1 = val
        this.depthId1 = val
        if (this.depthId1) {
          this.getDepthDetail()
        }
      ...
      //切换小组
      group: function (val) {
        this.$emit('clickIndex', 1);
        console.log('val')
        this.names.name2 = val
        this.depthId1 = val
        if (this.depthId1) {
          this.getDepthDetail()
        }
        this.member = null
        this.names.name3 = null
        ...
        /*console.log(this.groupOptions)
         for(let i=0;i<this.groupOptions;i++){
         if(this.groupOptions[i].id==val){
         console.log(this.groupOptions[i].name)
         }
         }*/
      },
      //切换组员
      member: function (val) {
        this.$emit('clickIndex', 2);
        this.members = val
        console.log(val)
        console.log(this.memberOptions)

        for (let i = 0; i < this.memberOptions.length; i++) {
          console.log(this.memberOptions[i].userIds)
          if (this.memberOptions[i].userIds == val) {
            this.names.name3 = this.memberOptions[i].roleName + '-' + this.memberOptions[i].name
            console.log(this.names.name3)
          }
        }
        this.sendToParent()
      }
    ...
    methods: {
      ...
      //初始获取部门树并保存vuex
      getDepthTree() {
        fetchTree(this.ruleForm).then(response => {
          //存储部门树
      ...
      getUsers() {
        ...
        getUserList(this.queryList).then(response => {
            ...
            if (this.member) {
              ...
              this.members = this.members.join(",")
              this.$emit("listenToChildEvent", [this.members, this.names])
      ...
      sendToParent: function () {
        this.$emit("listenToChildEvent", [this.members, this.names, this.clickIndex])
      },
